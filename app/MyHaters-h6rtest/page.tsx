  ...
  export default function AdminPage() {
    ...
    const deleteLoader = async (name: string) => {
      ...
      try {
        ...
        if (res.ok) {
          setLoaders(prev => prev.filter(l => l.name !== name));
        }
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4 max-w-md">
            ...
            {newKey && (
              <div className="bg-green-900/30 border border-green-800 rounded-lg p-3">
                ...
                {newKey.expires_at && (
                  <p className="text-green-600 text-xs mt-1">Expira: {fmt(newKey.expires_at)}</p>
                )}
              </div>
            )}
            <button
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4 max-w-md">
              ...
              {loadingConfig ? (
                ...
                <>
                  ...
                  />
                  {durResult && (
                    <p className={`text-xs ${durResult.ok ? "text-green-400" : "text-red-400"}`}>{durResult.msg}</p>
                  )}
                  <button
        ...
        {editingLoader && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-2xl space-y-4">
              ...
              {editingLoader.isNew && (
                <div>
                  <label className="text-gray-400 text-xs mb-1 block">Nombre del loader (solo letras, números, guiones)</label>
                  <input
                    value={editingLoader.name}
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={editingLoader.obfuscate}
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-xs">El script ejecutará igual pero el código será ilegible para quien lo vea</p>
                </div>
              </label>
